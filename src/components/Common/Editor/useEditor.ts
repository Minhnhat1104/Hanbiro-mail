...
const useEditor = () => {
  ...
  const { setFullscreen: setModalFullscreen } = useContext(BaseModalContext) || {}
  ...
  const defaultPlugins =
    "checklist lists advlist link image table code codesample insertdatetime emoticons fullscreen wordcount hanbiroclip"
  ...
  const defaultToolbar =
    "fontfamily fontsize | align  lineheight | bold italic underline | forecolor backcolor | code bullist numlist | link image table | codesample | insertdatetime emoticons | fullscreen | removeformat | print"
  ...
  const handleSetup = (editor) => {
    editor.on("FullscreenStateChanged", (data) => {
      const fullscreen = data?.state || false
      setModalFullscreen(fullscreen)
    })
  }
