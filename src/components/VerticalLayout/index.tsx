  ...
  return (
    <React.Fragment>
      <div id="layout-wrapper">
        {!(isMobile && isDetailView) && !isIframeMode && <Header />}
        <Sidebar isMobile={isMobile} />
        <div
          className={`main-content position-relative ${
            isShowSidebar ? "margin-left-sidebar" : "ms-0"
          }`}
        >
          {isShowSidebar && (isMobile || isVerticalTablet) && (
            <div
              className="layer-above position-absolute top-0 h-100 w-100"
              onClick={() => {
                if (isShowSidebar) {
